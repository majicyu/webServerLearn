...
void addfd(int epollfd,int fd,bool enabled_et){
    epoll_event event;
    event.data.fd = fd;
...
void et(epoll_event* events,int number,int epollfd,int listenfd){
    ...
    for(int i=0;i<number;i++){
        ...
        if(sockfd == listenfd){
            ...
            //这段代码不会被重复触发，所以循环读取数据，以确保socket读缓存中的所有数据读出
            printf("event trigger once\n");
            while(1){
                memset(buf,'\0',BUFFER_SIZE);
                int ret = recv(sockfd,buf,BUFFER_SIZE-1,0);
                if(ret < 0){
                    //对于非阻塞IO,下面条件成立表示数据已经全部读取完毕。此后，epoll就能再次触发sockfd上的EPOLLIN事件，以驱动下一次操作
